...
| # | Decision | Owner |
|---|----------|-------|
| 1 | ~~**The actual prices and caps.** The page proposes a structure; David owns the numbers.~~ **Answered 14 Aug 2026 by Lauren: $49/$99/$499 are final, and Full Coverage stays uncapped as a deliberate subsidy** — the exposure was sized (~−$1,256 from a ten-night month on one subscriber) and then accepted, rather than discovered later. **Two things did not close with it, and one of them has since closed on its own:** ~~the **"Save more than 50%" claim** ships on all three cards with no basis~~ — **gone 14 Aug 2026**, replaced by Kai's per-plan copy, none of which states a number, so the page now makes no savings claim at all. Still open and still David's: the **$195 boarding rate** every savings *figure* is computed against is inferred from his model as a residual, never stated in it. It no longer gates page copy, but it still drives the comparison table and every read on subsidy exposure. See `docs/open-asks.md` §1. | ~~prices, caps, claim~~ done · rate: David |
| 2 | ~~**Rollover** — what happens to unused walks.~~ **Answered 12 Aug 2026: use it or lose it, no rollover.** The FAQ states it plainly again as of 14 Aug 2026 — the question was dropped from the Figma on 13 Aug and the term survived only in the comparison table's "Unused days" row; it is now back as "What happens to services I don't use?", third in the list. The harshest term in the offer is disclosed in both places, and tests pin both. | ~~Facilitator / David~~ done |
...
| 5 | ~~**Geography** — the zip gate is inherited, not decided.~~ **Answered 14 Aug 2026 by Lauren: the market is Colorado, Maine, Massachusetts, New Hampshire, Oregon, Texas and Washington** — exactly what `MARKET_RANGES` has always encoded. The gate is unchanged and still encodes those seven states. **But the page no longer names them:** the Figma pass later that day hid the fineprint block on the out-of-market screen, so an out-of-market visitor is now told nothing about where we do operate and cannot tell a mistyped zip from a real gap. The *decision* is closed; the *disclosure* regressed. | ~~Undecided~~ done · disclosure: Lauren |
